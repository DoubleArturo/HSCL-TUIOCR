...
本專案是一個基於 Google Gemini 1.5 Pro/Flash 的智慧型發票辨識與稽核系統，專為台灣企業進項發票 (GUI) 設計。它採用 Local-First (本機優先) 架構，確保資料隱私，並提供高效的 AI 輔助校對流程。
...
1.  **匯入 (Import)**：
    ...
    *   (選用) 匯入 ERP 系統匯出的 `Excel/CSV` 檔，用於與 OCR 結果進行交叉比對。
2.  **AI 辨識 (OCR Processing)**：
    *   系統自動調用 Google Gemini API 進行多模態視覺辨識。
    *   **混合模型策略 (Hybrid Strategy)**：預設使用快速低成本的 **Gemini 1.5 Flash**。若信心分數過低或邏輯驗證失敗，自動升級為 **Gemini 1.5 Pro** 重試。
3.  **人工稽核 (Audit & Review)**：
...
*   **異常檢核頁面 (Error Review Page)**：
    ...
    *   **分割視窗**：左側原圖、右側欄位，方便快速修正錯誤，無需在海量發票中尋找。
