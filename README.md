...
1.  **匯入 (Import)**：
    *   使用者上傳 PDF 掃描檔或圖片 (JPG/PNG/**TIF/TIFF**)。TIF 格式會在前端自動轉換為 PNG 後送入 OCR。
...
2.  **AI 辨識 (OCR Processing)**：
    ...
    *   **文件類型分類 (Document Classification)**：AI 優先辨識文件類型，並強制將「銷貨單/出貨單/Packing List/出貨憑證」排除為 `非發票`，避免重複計帳。
3.  **人工稽核 (Audit & Review)**：
    *   使用者在「發票編輯器」或「異常檢核頁面」確認資料。
    *   AI 會自動標記潛在錯誤（如金額不平、統編錯誤）。信心度 ≥ 90% 的欄位不顯示數字，降低視覺干擾。
4.  **匯出 (Export)**：
    *   產生包含完整差異分析的 CSV 報表（UTF-8 with BOM），供會計人員修正 ERP 或存檔。
...
*   **買方統編驗證 (Buyer Tax ID)**：
    ...
    *   若 OCR 結果不符，將標記為錯誤，提醒使用者修正。
*   **重複發票偵測 (Deduplication)**：
    *   以「發票號碼 (Invoice Number)」為唯一鍵值。
    *   若專案中出現兩張相同號碼的發票，系統會發出警告，防止重複入帳。
*   **正確率計算 (Accuracy Calculation)**：
    *   以「實際有上傳附件的項目」為分母，排除 ERP 中無附件的資料列，反映真實比對率。

## 3. 支援的檔案格式 (Supported File Types)
...
*   **發票編輯器 (Invoice Editor)**：
    ...
    *   **右側表單**：欄位即時驗證。僅有信心分數低於 90% 且被標記的欄位才顯示警示，減少視覺干擾。
...
*   **Image Processing**: UTIF.js (TIF 轉檔)

## 6. Agent Skills (AI 開發指南)
