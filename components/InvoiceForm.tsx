...
const InvoiceForm: React.FC<Props> = ({
    ...
    return (
        <div className="w-[520px] min-w-[520px] shrink-0 flex flex-col border-l border-gray-200 bg-white z-30 shadow-[-10px_0_30px_rgba(0,0,0,0.05)] h-full">
            ...
            {/* 非發票 Warning Banner — below toolbar */}
            {(formData.document_type === '非發票' || (formData.error_code as string) === 'NOT_INVOICE') && (
                <div className="mx-4 mt-4 p-4 bg-amber-50 border-2 border-amber-400 rounded-xl flex items-start gap-3 shrink-0">
                    <Lucide.AlertOctagon className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
                    <div>
                        <p className="text-sm font-black text-amber-800 mb-1">非發票文件 — 無需稽核</p>
                        <p className="text-xs text-amber-700 leading-relaxed">
                            此文件被 AI 辨識為訂單出貨憑證、銷貨單或 Packing List，不屬於統一發票範疇。
                        </p>
                    </div>
                </div>
            )}
